...
bool priority_compare(const struct list_elem *X, const struct list_elem *Y, void *aux UNUSED){
  ...
  thread1 = list_entry(X, struct thread, elem);
  thread2=  list_entry(Y, struct thread, elem);
...
thread_set_priority (int new_priority) 
  {
     ...
     refresh_priority();
  //donate priority
   if(new_priority < old_priority && list_empty(&t->wait_on_lock))
    {
     t->priority=new_priority;
     thread_prempt();
    }
   else if(old_priority<t->priority)
	{donate_priority();}
...
thread_get_priority (void) 
{
    ...
    intr_set_level (old_level);
    //printf("Thread Priority in thread_get_priority : %d\n",tmp);
   return temp;  
}
...
{
  int64_t current_load_avg = load_avg;
  int64_t rounded_load_avg;
  current_load_avg = mult_mixed(current_load_avg, 100);
  rounded_load_avg = fp_to_int_round (current_load_avg);
  return rounded_load_avg;
  //int tmp = fp_to_int_round(load_avg * 100);
  //return tmp;
}
...
{
  enum intr_level old_level = intr_disable ();
  int tmp = fp_to_int_round( mult_fp(thread_current()->recent_cpu, 100));
  intr_set_level (old_level);
  return tmp;
}
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  list_push_back (&all_list, &t->allelem);
      ...
      // Initializations of variables for priority donation
  ...
  //printf("Thread Priority in init_thread : %d\n",t->priority);
}
...
void donate_priority (void)
  {
    ...
    struct lock *l ;
    l = t->wait_on_lock;
    while (l && check_depth < DEPTH_LIMIT)
      {
        check_depth++;
...
void refresh_priority (void)
{
  ...
  temp->priority = temp->initial_priority;
  if (list_empty(&temp->donations))
    {
      return;
    }
  struct thread *s = list_entry(list_front(&temp->donations),
        struct thread, donation_elem);
  if (s->priority > temp->priority)
    {
